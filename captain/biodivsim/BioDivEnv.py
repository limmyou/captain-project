...
class BioDivEnv(gym.Env):
    ...
    def _initEnv(self):
        ...
        raw_values = self.list_species_values
        max_value_coord = None
        species_values = None
        ...
        if isinstance(raw_values, np.floating):
            raw_values = [float(raw_values)]

        if isinstance(raw_values, np.ndarray):
            raw_values = raw_values.tolist()

        # 2) 값이 없거나 랜덤 민감도면 랜덤 생성
        if (isinstance(raw_values, (list, tuple)) and len(raw_values) == 0) or self.rnd_sensitivities:
            randval = np.random.choice([0.1, 10], self.n_species, p=[0.8, 0.2])
            species_values = randval / np.sum(randval) * self.n_species
            max_value_coord = np.random.choice(range(self.length), 2)

        # 3) [좌표, species_values] 형태면 분리
        elif (
            isinstance(raw_values, (list, tuple))
            and len(raw_values) == 2
            and isinstance(raw_values[0], (list, tuple, np.ndarray))
        ):
            max_value_coord = np.array(raw_values[0], dtype=float).reshape(-1)
            species_values = np.array(raw_values[1], dtype=float).reshape(-1)

        # 4) 그냥 species_values만 들어온 경우
        else:
            species_values = np.array(raw_values, dtype=float).reshape(-1)

        # 5) species_values 보정
        if species_values is None or species_values.size == 0:
            randval = np.random.choice([0.1, 10], self.n_species, p=[0.8, 0.2])
            species_values = randval / np.sum(randval) * self.n_species
        elif species_values.size == 1:
            species_values = np.repeat(species_values.item(), self.n_species)
        elif species_values.size != self.n_species:
            species_values = np.resize(species_values, self.n_species)

        # 6) max_value_coord가 정상 좌표일 때만 거리 가중치 적용
        if isinstance(max_value_coord, np.ndarray) and max_value_coord.size >= 2:
