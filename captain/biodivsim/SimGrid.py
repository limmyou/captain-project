...
class SimGrid(object):
    ...
    def simulate(self, n_years):
        """
        n_years 동안 간단하게 시뮬레이션을 진행하는 메서드.
        매 해마다 기후 업데이트와 알파 히스토그램 갱신 후 전체 개체수를 1% 감소시키는 예시입니다.
        """
        for year in range(n_years):
            # 기후 레이어 업데이트 (예: 매 해 일정 변화)
            self._climate_layer = self._climate_generator.updateClimate(self._climate_layer)
            # 교란 행렬 업데이트 및 알파 히스토그램 재계산
            self.updateAlphaHistogram()
            # 단순 예제로, 매 해 전체 개체수를 1% 감소시킵니다.
            self._h = self._h * 0.99
            print(f"[DEBUG] Year {year+1} simulated.")
    ...
    def initGrid(self, stateInitializer):
        ...
        print("[DEBUG] (SimGrid) self._h sum after getClimateTolerance:", self._h.sum())
