...
def plot_biodiv_env(pklfile=None, loaded_env=None, max_n_species=0, plot_titles=True):
    ...
    try:
        phylo_file = getattr(evolveGrid, "_phylo_file_name", None)

        if phylo_file and isinstance(phylo_file, str) and os.path.exists(phylo_file):
            ll = bt.loadNewick(phylo_file, absoluteTime=False)
        else:
            phylo_tree = getattr(evolveGrid, "_phylo_tree", None)
            if phylo_tree is None:
                print("[WARN] phylo tree missing, skip phylogeny plot")
                return fig_list, titles
            
            ll = convert_to_bt_tree(phylo_tree)

    except Exception as e:
        print(f"[WARN] plot_biodiv_env skipped: {e}")
        return fig_list, titles
...
def plot_env_state(
    ...
    outfile_name = "%s_year_%s" % (outfile, env.currentIteration)
