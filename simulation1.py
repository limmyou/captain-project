...
### [ADDED] reproject/resample을 위한 import
...
if __name__ == "__main__":
    ...
    ### [ADDED] 재현성 확보 (보고서/튜닝용)
    np.random.seed(42)

    #====================== 환경 및 경계 데이터 =======================
    site = "금오름"
    root = r"C:\Users\LG\Downloads\captain-project\gis_data"
    ...
    ### [CHANGED] SHP의 CRS를 기준으로 모든 raster를 같은 좌표계/격자에 맞춤
    if gdf.crs is None:
    ...
    #====================== (2) TIF 레이어 로드: SHP 격자에 정합(reproject) =======================
    tif_files = [
        "Ca_fin.tif", "OM_fin.tif", "pH_fin.tif", "CEC_fin.tif","EC_fin.tif"
    ]

    ### [ADDED] 레이어별 결과를 담을 dict
    layers = {}

    def read_to_target_grid(tif_path, dst_shape, dst_transform, dst_crs):
        """
        ### [ADDED]
        입력 TIF를 SHP 기준 격자(dst_shape, dst_transform, dst_crs)에 맞춰
        reproject + resample 해서 반환.
        """
    ...
    ### [ADDED] pH 점수화(중간이 최고): 6.5 근처가 1, 멀어질수록 감소
    def score_ph(ph_arr, mask=None, optimal=6.5, width=1.0):
        """
        width가 작을수록 최적 범위가 좁아짐.
        가우시안 형태: exp(-((ph-optimal)/width)^2)
        """
        ph = ph_arr.copy()
        if mask is not None:
    ...
    # ==================================================
    # MOSBY: 랜덤 살포 구역 + 효과 적용
    # =========================
    mosby_fraction = 0.10   # 살포 면적 비율(대상지 내부에서 대략 10%)
    mosby_factor   = 1.20   # 모스비 효과(그 구역의 K를 1.2배로)

    # 1) 대상지 내부(final_mask==1)에서만 랜덤 선택(대략 mosby_fraction)
    ...
    print(f"[MOSBY] target~{mosby_fraction*100:.1f}% | applied={applied_area}/{valid_area} ({applied_pct:.2f}%) | K x{mosby_factor}")
    # ==================================================
    ...
    for i in range(n_years):
        ...
        if (i + 1) in plot_years:
            ...
            # PDF 저장
            plot_env_state(env, wd=base_dir, outfile=os.path.basename(base_dir), file_format="one_pdf")
